...
| Automated checks | 270 tests, 1,184 assertions, strict TypeScript for `src/` and `scripts/`, 39 focused safety/evaluation tests | `bun run check` and `bun run eval` pass; static and coverage floors are enforced in CI |
...
10. [ ] Add direct API happy-path and provider-failure contracts for the highest-risk broker-backed route branches, then cover order reconciliation and concurrent reservation races without browser automation. Primary orders, mutations, recovery, and transactional capacity races have direct contracts. Signed option exercise/do-not-exercise actions cover exact quantity binding, drift rejection, sanitized provider failure, and retry. Strategy paper execution and stream callbacks remain.
