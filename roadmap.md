...
| Automated checks | 254 tests, 1,006 assertions, strict TypeScript for `src/` and `scripts/`, 39 focused safety/evaluation tests | `bun run check` and `bun run eval` pass; static and coverage floors are enforced in CI |
| Instrumented coverage | 95.67% functions and 96.67% lines across imported modules | Reviewed floors are 95% functions and 96% lines; `app.ts` is now 22.22% functions and 80.67% lines, while browser coverage is reported separately |
...
10. [ ] Add direct API happy-path and provider-failure contracts for the highest-risk broker-backed route branches, then cover order reconciliation and concurrent reservation races without browser automation. Standard equity preview/submission now covers successful receipts, replayed idempotency, sanitized placement failure and retry, plus concurrent turnover-cap enforcement; linked, basket, option, crypto, reconciliation, and other provider branches remain.
