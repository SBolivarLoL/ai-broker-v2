...
| --- | --- | --- |
| `bun run check` | Pass: 256 tests, 0 failures, 1,025 assertions across 62 files | Strict TypeScript for `src/` and `scripts/`, all Bun tests, and the coverage floor |
...
| Order policy and signatures | High for modules, standard equity, and basket routes | Preview, reservation, idempotency, replacement, cancellation, basket, option, short, and crypto tests plus direct equity and basket success/failure/replay contracts | Linked, option, crypto, reconciliation, and real broker race drills remain incomplete |
...
| HTTP/API composition | Moderate | Dependency-injected `createApp`, in-memory SQLite, fake Alpaca, common contracts, strategy lineage flow, and standard equity/basket success/failure/idempotency tests | Other broker-backed success, reconciliation, stream, and concurrency paths remain incomplete |
