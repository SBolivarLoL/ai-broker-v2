...
Last reviewed against `main` commit `1c918b3`: 2026-07-05.
...
| --- | --- | --- |
| `bun run check` | Pass: 259 tests, 0 failures, 1,051 assertions across 62 files | Strict TypeScript for `src/` and `scripts/`, all Bun tests, and the coverage floor |
...
| Git state at review | `main`, `dev`, `origin/main`, and `origin/dev` all at `1c918b3`; no open PR or stale feature branch before this increment |
...
| Order policy and signatures | High for modules, standard equity, basket, and standalone crypto routes | Preview, reservation, idempotency, replacement, cancellation, basket, option, short, and crypto tests plus direct equity, basket, and crypto success/failure/replay contracts and transactional equity/crypto capacity races | Linked, option, reconciliation, and real broker drills remain incomplete |
...
| HTTP/API composition | Moderate | Dependency-injected `createApp`, in-memory SQLite, fake Alpaca, common contracts, strategy lineage flow, and standard equity/basket/crypto success, failure, idempotency, and selected concurrency tests | Linked, option, reconciliation, stream, and remaining provider paths remain incomplete |
