...
const order = await alpaca.trading.orders.limit({ symbol, qty: 1, side, limitPrice: side === "buy" ? 0.01 : 1_000_000, clientOrderId });
...
try {
  ...
  for (let attempt = 0; attempt < 10 && status !== "canceled"; attempt++) {
    await Bun.sleep(250);
    status = (await alpaca.trading.orders.getOrderByClientOrderId({ clientOrderId })).status ?? "";
  }
  if (status !== "canceled") throw new Error(`Smoke order did not reconcile to canceled (status: ${status})`);
