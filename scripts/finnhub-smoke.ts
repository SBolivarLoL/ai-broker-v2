import { getFinnhubCompanyEnrichment } from "../backend/integrations/finnhub";
...
if (!keyConfigured) {
  ...
  if (!result.configured)
    throw new Error("Configured Finnhub key was not accepted");
  if (!result.profile && !result.earnings.length && !result.news.length)
    throw new Error(
      "Configured Finnhub returned no usable profile, earnings, or news enrichment",
    );
  ...
  if (
    result.sources.some((source) =>
      source.category === "news"
        ...
        : source.claimStatus !== "provider_record",
    )
  )
    throw new Error("Finnhub evidence claim labels are invalid");
  if (JSON.stringify(result).includes(process.env.FINNHUB_API_KEY!.trim()))
    throw new Error("Finnhub API key leaked into normalized output");
}
