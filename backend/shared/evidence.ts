...
export type CanonicalEvidence<
  T = unknown,
  C extends EvidenceCategory = EvidenceCategory,
> = {
  id: string;
  provider: string;
  sourceId: string;
...
function jsonValue(value: unknown): unknown {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value))
      throw new Error("Evidence data cannot contain non-finite numbers");
    return value;
  }
...
export function canonicalEvidenceUrl(value: string) {
  ...
  try {
    ...
    throw new Error("Evidence URL must be absolute");
  }
...
function normalizedEntityIds(value: EvidenceEntityIds) {
  ...
  if (symbol && !/^[A-Z0-9./-]{1,20}$/.test(symbol))
    throw new Error("Evidence symbol identifier is invalid");
  if (cik && !/^\d{1,10}$/.test(cik))
    throw new Error("Evidence CIK identifier is invalid");
  if (figi && !/^[A-Z0-9]{12}$/.test(figi))
    throw new Error("Evidence FIGI identifier is invalid");
  return {
    ...(symbol ? { symbol } : {}),
    ...(cik ? { cik: cik.padStart(10, "0") } : {}),
    ...(figi ? { figi } : {}),
  };
}
...
export type EvidenceDuplicateReason =
  "provider_source_id" | "canonical_url" | "exact_content";
export type EvidenceDuplicate = {
  keptId: string;
  discardedId: string;
  reason: EvidenceDuplicateReason;
};
...
function identityKey(record: CanonicalEvidence) {
  const ids = Object.entries(record.entityIds).sort(([a], [b]) =>
    a.localeCompare(b),
  );
  return ids.length
...
export function dedupeEvidence<T extends CanonicalEvidence>(records: T[]) {
  ...
  for (const record of records) {
    const sourceMatch = kept.find(
      (item) =>
        item.provider === record.provider && item.sourceId === record.sourceId,
    );
    const urlMatch = sourceMatch
      ? undefined
      : kept.find(
          (item) =>
    ...
    const reason: EvidenceDuplicateReason = sourceMatch
      ? "provider_source_id"
      : urlMatch
        ? "canonical_url"
        : "exact_content";
