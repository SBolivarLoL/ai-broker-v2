...
type FinnhubCoverageStatus =
  "missing_key" | "misconfigured" | EndpointCoverageStatus;
...
class FinnhubHttpError extends Error {
  constructor(
    ...
    super(`Finnhub returned HTTP ${status}`);
  }
}
...
const periodDate = (value: unknown) => {
  ...
  return /^\d{4}-\d{2}-\d{2}$/.test(result) &&
    Number.isFinite(new Date(`${result}T00:00:00.000Z`).getTime())
    ? result
    : null;
};
...
function earningsFrom(payload: unknown) {
  if (!Array.isArray(payload))
    throw new Error("Finnhub returned an invalid earnings payload");
  return payload
    .flatMap((raw) => {
      if (!raw || typeof raw !== "object") return [];
      const item = raw as Record<string, unknown>;
      const period = periodDate(item.period),
        actual = finite(item.actual),
        estimate = finite(item.estimate);
      if (!period || actual === null || estimate === null) return [];
      const quarter = finite(item.quarter),
...
function newsFrom(payload: unknown, symbol: string) {
  if (!Array.isArray(payload))
    throw new Error("Finnhub returned an invalid company-news payload");
  ...
  return payload
    .flatMap((raw) => {
      ...
      const url = webUrl(item.url),
        headline = text(item.headline, 400),
        seconds = finite(item.datetime);
      ...
      seen.add(evidenceId);
      const relatedSymbols = text(item.related, 300)
        .split(/[ ,]+/)
        .map((value) => value.toUpperCase())
        .filter((value) => symbolPattern.test(value))
        .slice(0, 12);
      return [
        {
          id: providerId,
...
export class FinnhubClient {
  ...
  constructor(options: FinnhubClientOptions = {}) {
    ...
    if (
      ...
      throw new Error("Finnhub client options are invalid");
  }
  ...
  private async fetchJson(url: string, key: string) {
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        const delay =
          error instanceof FinnhubHttpError && error.retryAfterMs !== null
    ...
    throw lastError instanceof Error
      ? lastError
      : new Error("Finnhub request failed");
  }
  ...
  private endpoint<T>(
    ...
    if (cached && cached.expiresAt > this.now())
      return Promise.resolve(cached.value as EndpointResult<T>);
    ...
    const request = (async () => {
      try {
        ...
        this.cache.set(cacheKey, {
          value: result,
          expiresAt: this.now() + successTtlMs,
        });
        return result;
      } catch (error) {
        const retrievedAt = new Date(this.now()).toISOString();
        const status =
          error instanceof FinnhubHttpError && error.status === 429
            ? "rate_limited"
            : "unavailable";
  ...
  async companyEnrichment(
    ...
    if (!symbolPattern.test(symbol))
      throw new Error(
        "A valid stock symbol is required for Finnhub enrichment",
      );
    if (
      !Number.isInteger(newsLookbackDays) ||
      newsLookbackDays < 1 ||
      newsLookbackDays > 30
    )
    ...
    if (!key)
      return empty(
        "missing_key",
        "Optional Finnhub enrichment is unavailable until FINNHUB_API_KEY is configured.",
      );
    ...
    if (earningsResult.status === "available") {
      try {
        ...
        warnings.push(
          "Finnhub earnings enrichment returned an invalid payload and was omitted.",
        );
      }
      if (earnings.length) {
        ...
        sources.push(
          canonicalEvidence({
            id: `finnhub:earnings:${symbol}`,
            provider: "finnhub",
            sourceId: `earnings:${symbol}:${latestPeriod}`,
      ...
      } else if (coverage.earnings === "available")
        warnings.push(
          "Finnhub returned no usable earnings surprises for this symbol.",
        );
    }
    ...
    if (newsResult.status === "available") {
      ...
      if (!news.length && coverage.news === "available")
        warnings.push(
          "Finnhub returned no usable company-news items for this window; do not infer that no material event exists.",
        );
    }
    ...
    for (const [label, endpointStatus] of Object.entries(endpointStatuses)) {
      if (endpointStatus === "rate_limited")
        warnings.push(
          `Finnhub ${label} enrichment is rate limited and was omitted.`,
        );
      else if (endpointStatus === "unavailable")
        warnings.push(
          `Finnhub ${label} enrichment is temporarily unavailable.`,
        );
    }
