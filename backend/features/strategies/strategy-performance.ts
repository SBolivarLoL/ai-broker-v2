...
type PerformanceBar = PriceBarInput;

const percent = (value: number, base: number) =>
  base > 0 ? (value / base) * 100 : null;
...
function filledOrder(order: StrategyPerformanceOrder) {
  ...
  const qty = finiteNumber(
    broker.filledQty ?? payload.filledQty ?? payload.qty,
  );
  const notional =
    finiteNumber(payload.notional) ?? (qty && price ? qty * price : null);
  if (
    status !== "filled" ||
    !symbol ||
    !filledAt ||
...
export function buildStrategyPerformance(input: {
  ...
  const fills = input.orders
    .map(filledOrder)
    .filter((fill): fill is NonNullable<typeof fill> => Boolean(fill))
    .sort((a, b) => a.filledAt.getTime() - b.filledAt.getTime());
  const approvalBudget = finiteNumber(input.run.config?.paperApproval?.budget);
  const totalBuyNotional = fills
    .filter((fill) => fill.side === "buy")
    .reduce((sum, fill) => sum + fill.notional, 0);
  const initialCapital =
    approvalBudget && approvalBudget > 0
      ? approvalBudget
      : input.run.budget > 0
        ? input.run.budget
        : totalBuyNotional;
  const warnings = [
    fills.length
      ? null
      : "No filled strategy paper orders are available for active-run performance yet.",
    initialCapital > 0
      ? null
      : "Strategy performance needs a positive budget or filled buy notional.",
  ...
  const symbols = [
    ...new Set([...input.run.symbols, ...fills.map((fill) => fill.symbol)]),
  ];
  const timeline = [
    ...new Set(
      Object.values(barsBySymbol).flatMap((bars) =>
        bars
          .filter((bar) => bar.timestamp.getTime() >= firstFillAt.getTime())
  ...
  if (!timeline.length) {
    warnings.push(
      "No crypto bars are available after the first strategy fill.",
    );
    return {
      performanceVersion: "strategy-performance-v1",
      generatedAt,
  ...
  const points = timeline.map((timestamp) => {
    ...
    for (const fill of fills) {
      if (applied.has(fill.id) || fill.filledAt.getTime() > at.getTime())
        continue;
      cash += fill.side === "sell" ? fill.notional : -fill.notional;
      units[fill.symbol] =
        (units[fill.symbol] ?? 0) +
        (fill.side === "sell" ? -fill.qty : fill.qty);
      applied.add(fill.id);
      lastPrices[fill.symbol] ??= fill.price;
      firstPrices[fill.symbol] ??= fill.price;
    }
    const holdingsValue = Object.entries(units).reduce(
      (sum, [symbol, qty]) => sum + qty * (lastPrices[symbol] ?? 0),
      0,
    );
    ...
    return {
      ...
      returnPercent: percent(equity - initialCapital, initialCapital),
    };
  });
  ...
  const buyAndHoldEquity =
    primaryStart && primaryEnd
      ? (initialCapital * primaryEnd) / primaryStart
      : null;
  const baselineSymbols = symbols.filter(
    (symbol) => firstPrices[symbol] && lastPrices[symbol],
  );
  const equalWeightEquity = baselineSymbols.length
    ? baselineSymbols.reduce(
        (sum, symbol) =>
          sum +
          (initialCapital / baselineSymbols.length) *
            (lastPrices[symbol]! / firstPrices[symbol]!),
        0,
      )
    : null;
  if (!primaryStart || !primaryEnd)
    warnings.push(
      "Buy-and-hold baseline needs start and end prices for the primary symbol.",
    );
  if (!baselineSymbols.length)
    warnings.push(
      "Equal-weight baseline needs start and end prices for at least one symbol.",
    );
  ...
  return {
    ...
    baselines: {
      cash: {
        ...
        returnPercent: 0,
        activeReturnPercent: current.returnPercent,
      },
      buyAndHold:
        buyAndHoldEquity === null
          ? null
          : {
              symbol: primarySymbol,
              equity: buyAndHoldEquity,
              returnPercent: percent(
                buyAndHoldEquity - initialCapital,
                initialCapital,
              ),
              activeReturnPercent:
                current.returnPercent! -
                percent(buyAndHoldEquity - initialCapital, initialCapital)!,
            },
      equalWeight:
        equalWeightEquity === null
          ? null
          : {
              symbols: baselineSymbols,
              equity: equalWeightEquity,
              returnPercent: percent(
                equalWeightEquity - initialCapital,
                initialCapital,
              ),
              activeReturnPercent:
                current.returnPercent! -
                percent(equalWeightEquity - initialCapital, initialCapital)!,
            },
    },
    points,
    warnings,
