...
export function parseStrategyIntervalMinutes(value: unknown) {
  ...
  if (!Number.isInteger(minutes) || minutes < 1 || minutes > 1_440)
    throw new Error(
      "Strategy schedule interval must be 1 to 1440 minutes, or 0 to disable scheduling",
    );
  return minutes;
}
...
export function normalizeStrategySchedule(config: unknown) {
  ...
  const intervalMinutes = parseStrategyIntervalMinutes(
...
export function strategyRunIsDue(
  run: ScheduledRun,
  now = new Date(),
  latestDecisionCreatedAt: string | null = null,
) {
  ...
  return (
    !latestDecision ||
    latestDecision.getTime() + schedule.intervalMinutes * 60_000 <=
      now.getTime()
  );
}

export function withNextStrategySchedule(
  config: unknown,
  completedAt = new Date(),
) {
  const schedule = normalizeStrategySchedule(config);
  if (!schedule || !isRecord(config))
    throw new Error("Strategy run is not scheduled");
  return {
    ...config,
    schedule: {
      ...
      nextRunAt: new Date(
        completedAt.getTime() + schedule.intervalMinutes * 60_000,
      ).toISOString(),
    },
  };
}
