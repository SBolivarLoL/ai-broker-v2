...
export function performancePoints(history: {
  timestamp: number[];
  equity: number[];
  profitLoss: number[];
  ...
  const length = Math.min(
    ...
    history.profitLoss.length,
    history.profitLossPct.length,
  );
  const cashflows = Object.values(history.cashflow ?? {}).filter(
    Array.isArray,
  ) as unknown[][];
  const externalCashFlows = Array.from({ length }, (_, index) =>
    cashflows.reduce((sum, values) => sum + Number(values[index] ?? 0), 0),
  );
  ...
  if (
    ...
    const delayedFunding = externalCashFlows.findIndex(
      (flow, index) =>
        index >= firstFunded &&
        flow > 0 &&
        Math.abs(flow - history.equity[firstFunded]) <=
          Math.max(1, history.equity[firstFunded] * 0.01),
    );
...
export function moneyWeightedReturn(points: PerformancePoint[]) {
  ...
  const years = (timestamp: number) =>
    Math.max(0, (timestamp - first.timestamp) / (365.25 * 86_400_000));
  const npv = (rate: number) =>
    flows.reduce(
      (sum, flow) => sum + flow.amount / (1 + rate) ** years(flow.timestamp),
      0,
    );
  let low = -0.9999,
    high = 1;
...
export function performanceSummary(points: PerformancePoint[]) {
  ...
  const returns = points
    ...
    .map(
      (point, index) =>
        (1 + point.profitLossPercent / 100) /
          (1 + points[index].profitLossPercent / 100) -
        1,
    )
    .filter(valid);
  ...
  const elapsedYears = Math.max(
    1 / 252,
    (points.at(-1)!.timestamp - points[0].timestamp) / (365.25 * 86_400_000),
  );
  const startingEquity = points[0].equity;
  const endingEquity = points.at(-1)!.equity;
  const totalReturn =
    startingEquity > 0 ? endingEquity / startingEquity - 1 : 0;
  ...
  return {
    ...
    annualizedReturnPercent:
      totalReturn > -1
        ? ((1 + totalReturn) ** (1 / elapsedYears) - 1) * 100
        : -100,
    sharpeRatio:
      variance > 0 ? (average / Math.sqrt(variance)) * Math.sqrt(252) : 0,
    positiveDaysPercent: returns.length
...
export function valueAtRisk95(equity: number, normalizedEquity: number[]) {
  const returns = normalizedEquity
    .slice(1)
    .map((value, index) => value / normalizedEquity[index] - 1)
    .filter(valid)
    .sort((a, b) => a - b);
  if (!returns.length || equity <= 0)
    return { valueAtRisk95Percent: 0, valueAtRisk95: 0 };
  const percentile = returns[Math.max(0, Math.ceil(returns.length * 0.05) - 1)];
  const lossPercent = Math.max(0, -percentile * 100);
  return {
    valueAtRisk95Percent: lossPercent,
    valueAtRisk95: (equity * lossPercent) / 100,
  };
}
...
export function stressTests(
  ...
  return [
    {
      name: "Broad selloff",
      detail: "All holdings fall 10%",
      estimatedLoss: invested * 0.1,
      resultingEquity: equity - invested * 0.1,
    },
    {
      name: "Severe bear market",
      detail: "All holdings fall 20%",
      estimatedLoss: invested * 0.2,
      resultingEquity: equity - invested * 0.2,
    },
    {
      name: "Largest holding shock",
      detail: largest ? `${largest.symbol} falls 25%` : "No invested position",
      estimatedLoss: ((equity * (largest?.percent ?? 0)) / 100) * 0.25,
      resultingEquity: equity * (1 - ((largest?.percent ?? 0) / 100) * 0.25),
    },
  ];
}
...
export function diversificationScore(
  ...
  const score = Math.round(
    Math.max(
      ...
      Math.min(
        100,
        100 - hhi * 100 - Math.max(0, largestPositionPercent - 10) * 1.5,
      ),
    ),
  );
  return {
