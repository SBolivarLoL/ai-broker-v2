...
export type StoredOutputCategory = {
  ...
  contents: string[];
  sourceIds: string[];
  retentionDecision: "application_lifetime" | "until_manual_deletion" | "until_replaced_or_deleted" | "retention_metadata_without_automatic_purge" | "selective_automatic_pruning_with_lineage_protection";
...
export const DATA_GOVERNANCE_SOURCES: DataGovernanceSource[] = [
  {
    ...
    storedOutputIds: ["operations_events", "order_authority", "advisor_plans", "trade_journal", "broker_account_ledger", "portfolio_snapshots", "strategy_experiments"],
  },
  {
    ...
    redistributionDecision: "blocked",
    liveUseDecision: "external_review_required",
    storedOutputIds: ["operations_events", "order_authority", "advisor_plans", "trade_journal", "research_runs", "portfolio_snapshots"],
  },
  {
    id: "alpaca_stock_sip",
    category: "market_data",
...
export const STORED_OUTPUT_CATEGORIES: StoredOutputCategory[] = [
  ...
  { id: "operations_events", tables: ["events"], contents: ["operational events", "local spans", "scheduler, retention, incident, and closed-beta workflow evidence", "bounded broker/market discrepancy fingerprints and recovery outcomes"], sourceIds: ["alpaca_paper_trading", "alpaca_equity_iex", "local_derived_analytics"], retentionDecision: "selective_automatic_pruning_with_lineage_protection", redistributionDecision: "internal_only", liveUseDecision: "operations_only" },
  ...
  { id: "research_runs", tables: ["research_runs"], contents: ["model output", "research metrics", "provider evidence and limitations", "generated company research with canonical replay hashes", "point-in-time comparable valuation and user-assumption scenario reports with canonical replay hashes"], sourceIds: ["alpaca_equity_iex", "alpaca_news_benzinga", "gdelt_doc_2", "finnhub_free_enrichment", "openfigi_v3", "sec_edgar", "treasury_fiscal_data", "bls_public_data", "fred_api", "bea_data_api", "openai_api", "local_derived_analytics"], retentionDecision: "selective_automatic_pruning_with_lineage_protection", redistributionDecision: "internal_only", liveUseDecision: "paper_only" },
  ...
  { id: "strategy_experiments", tables: ["strategy_backtests", "strategy_bar_datasets", "strategy_bars", "strategy_runs", "strategy_data_snapshots", "strategy_decisions", "strategy_orders", "strategy_metrics", "strategy_notes", "strategy_audit_log"], contents: ["immutable backtests and provenance", "versioned normalized crypto bars and dataset-quality evidence", "strategy configuration", "market snapshots and bounded order-book depth", "decisions", "paper orders", "metrics", "notes", "hash-chained audit"], sourceIds: ["alpaca_paper_trading", "alpaca_crypto_data", "local_derived_analytics"], retentionDecision: "selective_automatic_pruning_with_lineage_protection", redistributionDecision: "internal_only", liveUseDecision: "paper_only" },
];
...
export function buildDataGovernanceReport(generatedAt = new Date().toISOString()): DataGovernanceReport {
  ...
  return {
    ...
    runbook: [
      ...
      "Use the retention report and durable run evidence to verify bounded automatic pruning; audit chains and replay lineage remain protected.",
