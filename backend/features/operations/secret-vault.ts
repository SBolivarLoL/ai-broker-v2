...
function keyMaterial(secretKey: string) {
  if (secretKey.length < 32)
    throw new Error("SECRET_VAULT_KEY must be at least 32 characters");
...
export function decryptSecretValue(envelope: unknown, secretKey: string) {
  ...
  if (secret.keyDigest !== keyDigest(secretKey))
    throw new Error("Secret vault key does not match this envelope");
