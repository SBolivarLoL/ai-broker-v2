...
const actionLabels: Record<string, string> = {
  cashDividends: "Cash dividend",
  cashMergers: "Cash merger",
  forwardSplits: "Forward split",
  nameChanges: "Name change",
  partialCalls: "Partial call",
  redemptions: "Redemption",
  reorganizations: "Reorganization",
  reverseSplits: "Reverse split",
  rightsDistributions: "Rights distribution",
  spinOffs: "Spin-off",
  stockAndCashMergers: "Stock-and-cash merger",
  stockDividends: "Stock dividend",
  stockMergers: "Stock merger",
  unitSplits: "Unit split",
  worthlessRemovals: "Worthless removal",
};
...
const symbolFor = (action: any) =>
  String(
    action.symbol ??
      action.sourceSymbol ??
      action.oldSymbol ??
      action.acquireeSymbol ??
      "",
  ).toUpperCase();
const eventDate = (action: any) =>
  iso(
    action.exDate ??
...
function scopeFor(
  ...
  const lists = watchlists
    .filter((list) =>
      list.assets.some((asset) => symbols.includes(asset.symbol.toUpperCase())),
    )
    .map((list) => ({ id: list.id, name: list.name }));
  return { portfolio, watchlists: lists };
}

export function monitoringNews(
  articles: any[],
  ...
  return articles
    ...
    .filter(
      (article) =>
        article.createdAt &&
        (article.relevance.portfolio || article.relevance.watchlists.length),
    )
    .sort(
      (a, b) =>
        Number(b.relevance.portfolio) - Number(a.relevance.portfolio) ||
...
function holdingImpact(type: string, action: any, quantity: number | null) {
  ...
  if (type === "cashDividends" && Number.isFinite(Number(action.rate)))
    return {
      kind: "cash",
      estimatedCash: quantity * Number(action.rate),
      message: `Estimated cash based on ${quantity} currently held shares.`,
    };
  if (
    ["forwardSplits", "reverseSplits"].includes(type) &&
    Number(action.oldRate) > 0 &&
    Number(action.newRate) > 0
  ) {
    const estimatedQuantity =
      (quantity * Number(action.newRate)) / Number(action.oldRate);
    return {
      kind: "quantity",
      estimatedQuantity,
      message: `Estimated quantity changes from ${quantity} to ${estimatedQuantity}; total market value is not implied to change.`,
    };
  }
  return {
    kind: "review",
    message:
      "This holding may be affected; final terms and broker accounting require review.",
  };
}

export function monitoringCorporateActions(
      ...
      (actions ?? []).map((action) => {
        ...
        const relatedSymbols = [
          ...new Set(
            [
              symbol,
              action.newSymbol,
              action.acquirerSymbol,
              action.alternateSymbol,
            ]
              .filter(Boolean)
        ...
        return {
          ...
          rate: Number.isFinite(Number(action.rate))
            ? Number(action.rate)
            : null,
          oldRate: Number.isFinite(Number(action.oldRate))
            ? Number(action.oldRate)
            : null,
          newRate: Number.isFinite(Number(action.newRate))
            ? Number(action.newRate)
            : null,
          relevance: scopeFor(relatedSymbols, held, watchlists),
          impact: holdingImpact(type, action, quantity),
        };
      }),
    )
    .filter(
      (action) =>
        action.symbol &&
        action.eventDate &&
        (action.relevance.portfolio || action.relevance.watchlists.length),
    )
    .sort(
      (a, b) =>
        Number(b.relevance.portfolio) - Number(a.relevance.portfolio) ||
...
export function monitoringSecFilings(
  ...
  return alerts
    ...
    .sort(
      (a, b) =>
        secImportanceRank[b.importance] - secImportanceRank[a.importance] ||
...
const eventKind = (text: string) => {
  if (/earnings|revenue|guidance|quarter|results/i.test(text))
    return "earnings";
  if (/dividend|distribution/i.test(text)) return "dividend";
  if (/merger|acquisition|acquire|takeover/i.test(text)) return "merger";
  if (/fda|lawsuit|regulat|investigation|antitrust/i.test(text))
    return "regulatory";
  if (/upgrade|downgrade|price target|analyst/i.test(text)) return "analyst";
  return "company_update";
};

export function monitoringEventClusters(
  news: ReturnType<typeof monitoringNews>,
  actions: ReturnType<typeof monitoringCorporateActions>,
  filings: ReturnType<typeof monitoringSecFilings> = [],
) {
  const entries = [
    ...news.flatMap((article) =>
      article.relevantSymbols.map((symbol) => ({
        symbol,
        kind: eventKind(`${article.headline} ${article.summary}`),
    ...
    ...actions.map((action) => ({
      ...
      kind: action.type === "cashDividends" ? "dividend" : "corporate_action",
      at: action.eventDate!,
      label: action.label,
    ...
    ...filings.map((filing) => ({
      ...
      kind: "sec_8k",
      at: `${filing.filed}T00:00:00.000Z`,
      label: `Item ${filing.primaryItem.code}: ${filing.primaryItem.label}`,
      source: "sec_8k" as const,
      id: filing.id,
    })),
  ];
  const grouped = new Map<string, typeof entries>();
  for (const entry of entries) {
    const key = `${entry.symbol}:${entry.kind}`,
    ...
    .map(([key, timeline]) => {
      ...
      return {
        ...
        latestAt: timeline
          ...
          .sort()
          .at(-1)!,
        timeline: timeline.sort((a, b) => b.at.localeCompare(a.at)).slice(0, 5),
      };
    })
