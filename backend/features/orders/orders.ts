...
export const Preview = z
  .object({
    ...
    simulation: z
      .object({
        ...
        resultingCash: z.number(),
        resultingPositionPercent: z.number(),
  ...
  .superRefine((preview, context) => {
    if (
      ...
      (preview.type !== "market" || !preview.notional)
    )
      context.addIssue({
        code: "custom",
        message: "Notional orders must be market orders",
      });
    if (preview.type === "limit" && !preview.limitPrice)
      context.addIssue({ code: "custom", message: "Limit price is required" });
    if (preview.type === "stop" && !preview.stopPrice)
      context.addIssue({ code: "custom", message: "Stop price is required" });
    if (
    ...
    if (preview.type === "trailing_stop" && !preview.trailPercent)
      context.addIssue({
        code: "custom",
        message: "Trail percent is required",
      });
    if (preview.extendedHours && preview.type !== "limit")
      context.addIssue({
        code: "custom",
        message: "Extended hours requires a limit order",
      });
    if (
      preview.orderClass === "bracket" &&
      (!preview.takeProfitPrice || !preview.stopLossPrice)
    )
      context.addIssue({
        code: "custom",
        message: "Bracket legs are required",
      });
    if (
      preview.orderClass === "oco" &&
      (!preview.takeProfitPrice || !preview.stopLossPrice)
    )
      context.addIssue({ code: "custom", message: "OCO legs are required" });
    if (
      preview.orderClass === "oto" &&
      Number(Boolean(preview.takeProfitPrice)) +
        Number(Boolean(preview.stopLossPrice)) !==
        1
    )
      context.addIssue({
        code: "custom",
        message: "Exactly one OTO leg is required",
      });
  });
...
export function verifyPreview(token: string, secret: string, now = Date.now()) {
  const preview = Preview.parse(
    verifyToken(token, secret, "Invalid preview token"),
  );
...
export async function verifyPreviewFresh<T>(
  ...
  secret: string,
  validate: (
    intent: Omit<Preview, "simulation" | "expiresAt">,
  ) => T | Promise<T>,
  now = Date.now(),
) {
