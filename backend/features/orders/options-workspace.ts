...
export function blackScholesGreeks(
  ...
  const thetaAnnual =
    -(spot * normalPdf(d1) * volatility) / (2 * rootT) +
    (type === "call"
      ? -riskFreeRate * strike * Math.exp(-riskFreeRate * years) * normalCdf(d2)
      : riskFreeRate *
        strike *
        Math.exp(-riskFreeRate * years) *
        normalCdf(-d2));
  const rho =
    (type === "call"
      ? strike * years * Math.exp(-riskFreeRate * years) * normalCdf(d2)
      : -strike * years * Math.exp(-riskFreeRate * years) * normalCdf(-d2)) /
    100;
...
export function optionChainDto(
  contracts: any[],
  ...
  const rows = contracts
    .map((contract) => {
      ...
      return {
        ...
        volume: finite(snapshot.dailyBar?.v),
        bid,
        ask,
        midpoint,
        spreadBps,
        ...
        greeks: snapshot.greeks
          ? {
              delta: finite(snapshot.greeks.delta),
              gamma: finite(snapshot.greeks.gamma),
              theta: finite(snapshot.greeks.theta),
              vega: finite(snapshot.greeks.vega),
              rho: finite(snapshot.greeks.rho),
            }
          : null,
      };
    })
  ...
  return {
    ...
    selected: selected
      ...
      : null,
    account: {
      approvedLevel: Number(account.optionsApprovedLevel ?? 0),
...
export function optionPortfolioGreeks(
  positions: any[],
  snapshots: Record<string, any>,
  contracts: any[],
  ...
  const shockPnl = (shock: number) =>
    legs.reduce((sum, leg) => {
      const spot = underlyingPrices[leg.underlying ?? ""],
        move = spot * shock;
      return (
        sum +
        (leg.greeks && Number.isFinite(spot)
          ? leg.greeks.delta * move + 0.5 * leg.greeks.gamma * move * move
          : 0)
      );
    }, 0);
  return {
    ...
    scenarios: [
      { name: "Underlying −10%", estimatedPnl: shockPnl(-0.1) },
      { name: "Underlying −5%", estimatedPnl: shockPnl(-0.05) },
      { name: "Underlying +5%", estimatedPnl: shockPnl(0.05) },
      { name: "Underlying +10%", estimatedPnl: shockPnl(0.1) },
      { name: "IV +10 points", estimatedPnl: total("vega") * 10 },
