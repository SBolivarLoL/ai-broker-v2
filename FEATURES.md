...
- The 273-test suite includes direct request-boundary contracts and enforces strict TypeScript for `src/` and `scripts/` plus 95% function and 96% line coverage across imported TypeScript modules. `src/app.ts` is instrumented at 44.60% of functions and 86.11% of lines; primary orders, mutations, option actions, strategy paper execution, concurrent capacity, recovery, and terminal trade-stream reconciliation are covered. The browser client is validated separately rather than included in that percentage.
