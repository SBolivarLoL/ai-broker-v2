...
- The 270-test suite includes direct request-boundary contracts and enforces strict TypeScript for `src/` and `scripts/` plus 95% function and 96% line coverage across imported TypeScript modules. `src/app.ts` is instrumented at 43.30% of functions and 84.23% of lines; primary orders, mutations, option-position actions, concurrent capacity, and recovery reconciliation are covered. Strategy paper execution still needs direct provider-boundary contracts. The browser client is validated separately rather than included in that percentage.
