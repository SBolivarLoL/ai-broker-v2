...
- The 256-test suite includes direct request-boundary contracts and enforces strict TypeScript for `src/` and `scripts/` plus 95% function and 96% line coverage across imported TypeScript modules. `src/app.ts` is instrumented at 31.07% of functions and 82.12% of lines; standard equity and basket order success, retry/partial failure, idempotency, and capacity are covered, but many other broker-backed route branches remain uncovered. The browser client is validated separately rather than included in that percentage.
