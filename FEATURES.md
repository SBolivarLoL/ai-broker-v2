...
- The 254-test suite includes direct request-boundary contracts and enforces strict TypeScript for `src/` and `scripts/` plus 95% function and 96% line coverage across imported TypeScript modules. `src/app.ts` is instrumented at 22.22% of functions and 80.67% of lines; standard equity order success, retry, idempotency, and concurrent capacity are covered, but many other broker-backed route branches remain uncovered. The browser client is validated separately rather than included in that percentage.
