...
- `src/app.ts` remains a 2,505-line request/composition module; `src/index.html` is a roughly 256 KB single-file client; `src/store.ts` still contains all repositories. The small `src/server.ts` entry and `src/migrations.ts` schema boundary are separate, but per-domain route, repository, and UI maintenance remain concentrated.
- The 265-test suite includes direct request-boundary contracts and enforces strict TypeScript for `src/` and `scripts/` plus 95% function and 96% line coverage across imported TypeScript modules. `src/app.ts` is instrumented at 41.45% of functions and 83.96% of lines; standard equity, linked, basket, option, standalone crypto, concurrent-capacity, and recovery-reconciliation paths are covered. Replacement/cancellation, option-position actions, and strategy paper execution still need direct provider-boundary contracts. The browser client is validated separately rather than included in that percentage.
