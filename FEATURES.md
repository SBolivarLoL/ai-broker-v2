...
- The 259-test suite includes direct request-boundary contracts and enforces strict TypeScript for `src/` and `scripts/` plus 95% function and 96% line coverage across imported TypeScript modules. `src/app.ts` is instrumented at 35.14% of functions and 83.13% of lines; standard equity, basket, and standalone crypto order success, retry/partial failure, idempotency, and relevant capacity paths are covered, but linked, option, reconciliation, and other broker-backed branches remain incomplete. The browser client is validated separately rather than included in that percentage.
