...
- **AI Portfolio Copilot:** the OpenAI Agents SDK uses read-only portfolio and latest-price tools to produce three structured ideas with an action, thesis, risk, invalidation condition, and confidence. Strict schemas, timeouts, scope and misleading-claim guardrails constrain the run. It has no order tool; users retain control through the existing confirmation flow.
