...
| `bun test tests/integrations/alpaca-stream-control.test.ts tests/markets/stock-stream.test.ts tests/markets/service.test.ts tests/orders/runtime.test.ts tests/http/app.test.ts` | Pass on 2026-09-06: 71 tests, 0 failures, 603 assertions across 5 files | Exact Alpaca SDK 0.2.0 send/ping race containment, unrelated-error propagation, real-SDK authentication and resubscription after reconnect, shared stock subscriptions, stream and timer teardown, composed scheduler ownership, and idempotent start/stop/restart behavior without stale symbols |
...
and degraded runs, account/position arithmetic, malformed values, same-time bar
revisions, 20-symbol/25-order bounds, newer versus ambiguous order evidence,
local recovery verification, overlap coalescing, sanitized unexpected-failure
events, GET reporting, POST execution, authorization-family placement, and
scheduler configuration. A separate order-runtime regression proves that a
...
The explicit time-taxonomy inventory extends the required canonical-evidence
constructor contract to a repository-level static gate. It currently finds 10
normalized exported DTO type literals and classifies 13 reviewed inputs, leaves,
persistence records, or taxonomy types; new unclassified time-bearing exports
and stale exceptions fail `bun run check`. Focused local-response contracts prove
trade-journal, decision-audit, operations, strategy-list, and stock-stream status
envelopes report unavailable provider retrieval without inventing observation or
publication time, and use one identical `asOf`, server-response field, and nested
...
The volatility-targeted-trend slice adds a deterministic plugin/schema
regression and extends direct composed-application and maintained browser
contracts. The plugin test proves warm-up behavior, a one-complete-bar
volatility lag, low-volatility scaling, a hard exposure ceiling, bounded
upward exposure changes, immediate high-volatility reduction, and explicit
non-leverage thresholds. Direct API coverage proves strict invalid-parameter
rejection before provider work, stored-dataset execution without another
provider read, immutable direct backtest-to-shadow lineage, and fail-closed
409 responses at both paper-protocol and paper-approval routes. The runtime
repeats the shadow-only gate before paper drafting or account/broker work. The
browser contract verifies all new bounded fields and conservative presets.
...
The regime-filtered-mean-reversion slice adds one deterministic plugin/schema
regression, one direct composed-application contract, and maintained browser
assertions. The plugin regression proves that trend return, realized
volatility, and average dollar volume all end one complete bar before the
current-close z-score; the decision bar's extreme volume cannot enter its own
liquidity filter. It separately proves compatible entry, incompatible-regime
exit, close-based stop loss, mean-threshold exit, exact maximum holding period,
minimum liquidity, missing-volume failure, a bounded exposure cap, and
identical sequential/isolated-tick state reconstruction. Direct API coverage
proves invalid holding configuration fails before provider work, exact reviewed
backtest-to-shadow lineage, 409 paper-protocol/approval responses, and a runtime
`strategy_shadow_only` block before any order attempt even when persisted state
is tampered into paper mode. The browser contract verifies the lagged regime,
liquidity, stop, holding, and conservative-preset controls. These checks do not
establish that the regime improves returns, paper evidence, or promotion
readiness.
...
- Direct API tests own common authorization, origin, body-size, JSON-object envelope parsing, malformed percent-encoding, status, schema, 404 and error-sanitization contracts without starting streams or a server port. The focused `tests/shared/http.test.ts tests/http/app.test.ts` run passed 48 tests and 444 expectations on 2026-09-05, including null, primitive, and array body rejection before order/backtest route work, malformed dataset-path encoding, valid encoded dataset lookup, and the existing sanitized dataset-provider failure. Broker-backed happy paths still need incremental route coverage.
...
- A maintained 2026-07-13 Playwright 1.61.1 Chromium suite passed three tests
  ...
  the single `aria-current="page"` state, changes the order filter through
  keyboard type-ahead, and verifies the resulting table status. It triggers a
...
- On 2026-07-12, the new reconciliation service ran once against the configured
  ...
  market symbols were omitted, there were no open orders to detail-check, and
  the run produced one completed event with zero discrepancy or recovery
  events. Output was restricted to schema/status/check/count/code fields; no
  credentials, balances, symbols, holdings, or order identifiers were logged.
  This proves one read-only endpoint-reconciliation sample, not long-running
  scheduled operation, a second-provider comparison, or entitlement approval.
...
- A 2026-07-12 headed browser pass rendered comparison v2 through the production
  Strategy Lab against two locally persisted backtests created from one
  configured read-only Alpaca crypto dataset. Both 160-point equity-return and
  drawdown charts aligned, both experiment cards exposed walk-forward bands,
  holdout results, decision counts, and eight total promotion blockers, and the
  result region returned `aria-busy="false"`. At 1440×1000 the two charts and
  cards rendered side by side; at 768×1000 and 390×844 they collapsed to one
  column. The final mobile cards and charts were each 330 CSS pixels wide, with
  no page or result-region horizontal overflow; the browser console reported no
  errors. Dirty checkout provenance correctly made the artifacts non-rankable.
  The pass created only local dataset/backtest evidence and made no OpenAI,
  order, policy, broker-account, or live-trading mutation.
...
- Backtest comparison v2 reports metric rows only with explicit compatibility checks for period, symbols, timeframe, dataset hash, friction model, baseline set, code/provider identity, and feed. Pure unit and direct API contracts cover compatible and incompatible cohorts, shared endpoint-preserving downsampling to at most 160 chart points, exact timestamp alignment, normalized full-sample and walk-forward uncertainty, final-holdout/leakage evidence, decision counts, and fail-closed promotion blockers.
