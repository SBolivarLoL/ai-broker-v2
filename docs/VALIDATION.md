...
| ------------------ | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `bun run check`    | Pass: 331 tests, 0 failures, 1,487 assertions across 81 files                     | Strict TypeScript for `backend/`, `tests/`, and `scripts/`, all Bun tests, and the coverage floor |
...
| Git baseline  | `main`, `dev`, `origin/main`, and `origin/dev` at `9c7c373`; no open pull request at change start                |
...
| Provider normalization      | Good with fixtures                                 | SEC, macro, GDELT, Finnhub, OpenFIGI, market-data fallback tests, canonical time-provenance tests, equity market DTO time-provenance tests, local provider-health evidence, plus deliberate live Alpaca/SEC reads           | Live provider contracts are not run in CI, not every DTO has the explicit time taxonomy, and point-in-time fundamentals are not persisted |
...
language against `9c7c373` plus fresh command output.
...
- Canonical evidence, crypto Strategy Lab market DTOs, company-market snapshots, and equity quote/bar stream DTOs preserve retrieval/server-response timestamps separately from provider observation timestamps; official macro evidence records effective periods for dated, monthly, and quarterly observations.
