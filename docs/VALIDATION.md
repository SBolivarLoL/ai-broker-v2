...
| ------------------ | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `bun run check`    | Pass: 332 tests, 0 failures, 1,494 assertions across 81 files                     | Strict TypeScript for `backend/`, `tests/`, and `scripts/`, all Bun tests, and the coverage floor |
...
| Git baseline  | `main`, `dev`, `origin/main`, and `origin/dev` at `54a16dc`; no open pull request at change start                |
...
| Provider normalization      | Good with fixtures                                 | SEC, macro, GDELT, Finnhub, OpenFIGI, market-data fallback tests, canonical time-provenance tests, equity and multi-asset market DTO time-provenance tests, local provider-health evidence, plus deliberate live Alpaca/SEC reads           | Live provider contracts are not run in CI, not every DTO has the explicit time taxonomy, and point-in-time fundamentals are not persisted |
...
language against `54a16dc` plus fresh command output.
...
- Canonical evidence, crypto Strategy Lab market DTOs, company-market snapshots, equity quote/bar stream DTOs, and multi-asset market DTOs preserve retrieval/server-response timestamps separately from provider observation timestamps; official macro evidence records effective periods for dated, monthly, and quarterly observations.
