...
- Company price, bid/ask spread, volume, daily bars, SPY/QQQ/DIA comparison, source timestamps, news, eligibility badges, and logo fallback. The company-market snapshot, equity quote/bar stream DTOs, and multi-asset index/FX/crypto DTO distinguish provider observation time from retrieval and server response time.
...
- Canonical evidence, crypto Strategy Lab market DTOs, company-market snapshots, equity quote/bar stream DTOs, and the multi-asset market DTO distinguish provider observation time from retrieval and server response time; official macro evidence also records effective periods for record dates, months, and quarters.
