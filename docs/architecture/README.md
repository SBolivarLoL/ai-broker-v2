...
deterministic valuation math and normalized comparable-v3/scenario-v3 evidence
...
Historical scenario child runs embed the verified parent replay, original
ordered assumptions, and v3 memo. Replay validates both manifests, replays the
parent, recomputes the memo at its original calculation time, and compares the
canonical result without a provider call. The v3 contract calls the selected
market input `referencePrice`; `research.js` labels either the latest returned
