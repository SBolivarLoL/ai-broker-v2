...
| Research data         | SEC, Alpaca/IEX, Treasury, BLS, optional FRED/BEA/Finnhub, GDELT, OpenFIGI, and optional OpenAI                                                                                                                                                                                                               | The registry covers 16 sources and all 23 SQLite tables through 12 output categories; provider health, strategy dataset quality, canonical evidence time provenance, and migrated broker account/position/order-state/account-activity/watchlist/asset-reference/portfolio-performance/portfolio-risk/portfolio-exposure/portfolio-scenario/portfolio-rebalance/portfolio-snapshot/portfolio-optimizer, single-symbol quote/market-monitoring/company-market/options/market-workspace/multi-asset/GDELT/Finnhub/OpenFIGI/SEC EDGAR/official-macro DTO time provenance are visible from local evidence, while full DTO migration, retention enforcement, and external entitlement review remain open |
...
3. [ ] Persist the canonical evidence used by important research/strategy decisions so later replays do not depend on mutable provider responses. Immutable strategy experiments retain normalized datasets, snapshots, lineage, and decision evidence; completed company-research reports retain their canonical source set. Saved Advisor plans retain exact cited proposal/review snapshots with simulation authority and hashes. Historical comparable and user-assumption scenario runs now persist bounded canonical v3 artifacts; scenario replay embeds and verifies the exact parent replay, original assumptions, and deterministic recomputation with zero providers. This remains open for provider-free reevaluation of generated company research and any other material decision that still requires mutable provider reads.
