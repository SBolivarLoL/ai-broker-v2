...
7. [ ] Add a visible data-coverage panel for each research report, portfolio calculation, and strategy run: expected inputs, received inputs, omitted inputs, freshness, and impact on conclusions. Portfolio optimizer, portfolio scenario, and constrained-rebalance responses plus their visible status/evidence lines now expose this contract; the remaining workspaces still need equivalent visible detail.
