...
| Strategy research     | Nine deterministic plugins, immutable linked backtests, versioned long-history crypto bars, rolling/anchored train-only walk-forward evaluation with final holdouts and regime slices, shadow/paper runs, traces and attribution                      | Exact lineage, per-fold leakage checks, final-holdout isolation, and report-only regime summaries are enforced; uncertainty, richer metrics, and long paper evidence remain open                                  |
