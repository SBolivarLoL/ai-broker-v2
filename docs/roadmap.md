...
3. [ ] Persist the canonical evidence used by important research/strategy decisions so later replays do not depend on mutable provider responses. Immutable strategy experiments already retain normalized datasets, snapshots, lineage, and decision evidence; completed company-research reports retain their canonical source set. Saved Advisor plans retain only exact cited allow-listed proposal/review snapshots with simulation authority and hashes. Historical comparable valuation runs now persist a bounded canonical v3 report plus replay manifest in `research_runs`; provider-free replay verifies every source hash and both time cutoffs. This remains open for persisted user-assumption valuation scenarios and any other material research decision that still requires mutable provider reads.
