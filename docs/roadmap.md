...
| Strategy research     | Nine deterministic plugins, immutable linked backtests, versioned long-history crypto bars, rolling/anchored train-only walk-forward evaluation with final holdouts, regime slices, trade metrics, uncertainty ranges, compatible cohort comparison, shadow/paper runs, traces and attribution                      | Exact lineage, per-fold leakage checks, final-holdout isolation, report-only regime summaries, deterministic trade metrics, not-rankable bootstrap uncertainty evidence, paper-friction calibration, and compatibility warnings are enforced; long paper evidence remains open                                  |
