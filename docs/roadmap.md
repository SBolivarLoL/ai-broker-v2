...
7. [ ] Add a visible data-coverage panel for each research report, portfolio calculation, and strategy run: expected inputs, received inputs, omitted inputs, freshness, and impact on conclusions. Every selected Strategy Lab run dashboard, generated company research, comparable and scenario valuations, the account-activity/FIFO ledger, and portfolio risk, exposure, snapshots, performance, optimizer, scenario, and constrained-rebalance responses now expose this contract through visible panels or status/evidence lines; other advisor and research reports still need equivalent visible detail.
