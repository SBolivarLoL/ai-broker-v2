...
Code baseline reviewed: `main` at `e38a21a` on 2026-07-10.
...
- Alpaca paper account, positions, orders, activities, watchlists, market clock, calendar, IEX market data, single-symbol quotes, market monitoring, company-market snapshots, market workspace DTOs, and entitled multi-asset market snapshots. Account/position state, managed orders and nested legs, order-list recovery state, cancel-all previews, watchlists and their asset entries, market-workspace aggregates, and the other migrated provider DTOs expose explicit observation/retrieval/server-response provenance. Watchlist observation uses the provider update time while nested asset observations remain null; account and position observations also remain null because those endpoints do not provide an event timestamp.
