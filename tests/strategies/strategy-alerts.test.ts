...
import { buildStrategyAlerts } from "../../backend/features/strategies/strategy-alerts";
...
const run = {
  ...
  budget: 250,
  config: {
    paperApproval: { budget: 250 },
    alerts: {
      drawdownPercent: 8,
      dailyTurnoverPercent: 40,
      slippageBps: 25,
      repeatedSlippageCount: 2,
  ...
  const result = buildStrategyAlerts({
    ...
    decisions: [
      ...
      },
      {
        ...
        decision: "block",
        reason: "broker",
        riskChecks: { reasons: ["broker_order_rejected"] },
    ...
    traces: [
      {
        traceId: "trace-stale",
        snapshots: [{ stale: true }, { stale: false }],
      },
    ],
    orders: [
      {
        paperOrderId: "paper-rejected",
        status: "rejected",
        payload: {},
        createdAt: "2026-06-24T11:58:00.000Z",
...
test("detects runaway turnover, repeated slippage and reconciliation drift", () => {
    ...
    orders: [
      {
        ...
        payload: {
          ...
          notional: 80,
          referencePrice: 100,
          broker: { status: "filled", filledAvgPrice: 100.5 },
        },
      ...
      },
      {
        ...
        payload: {
          ...
          notional: 70,
          referencePrice: 100,
          broker: { status: "filled", filledAvgPrice: 100.4 },
        },
        createdAt: "2026-06-24T11:00:00.000Z",
        updatedAt: "2026-06-24T11:01:00.000Z",
      },
      {
        paperOrderId: "paper-drift",
        status: "accepted",
  ...
  expect(
    result.alerts.find((alert) => alert.code === "runaway_turnover")?.evidence,
  ).toMatchObject({ turnoverPercent: 60, orderCount: 3 });
});
