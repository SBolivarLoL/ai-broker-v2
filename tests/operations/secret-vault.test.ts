...
import {
  ...
  encryptSecretValue,
  secretMetadata,
} from "../../backend/features/operations/secret-vault";
...
test("encrypts and decrypts secret values without storing plaintext", () => {
  ...
  expect(() =>
    decryptSecretValue(envelope, "wrong-secret-key-wrong-secret-key-1234"),
  ).toThrow("Secret vault key does not match");
});
...
test("exposes only encrypted secret metadata", () => {
  ...
  expect(
    secretMetadata("OPENAI_API_KEY", envelope, "admin@example.com"),
  ).toMatchObject({
    name: "OPENAI_API_KEY",
    algorithm: "aes-256-gcm",
  ...
  expect(
    secretMetadata("OPENAI_API_KEY", envelope).ciphertextBytes,
  ).toBeGreaterThan(0);
});
