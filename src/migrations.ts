...
function hasColumn(db: Database, table: string, column: string) {
  return (db.query(`PRAGMA table_info(${table})`).all() as { name: string }[]).some(item => item.name === column);
}
...
export const SCHEMA_MIGRATIONS: readonly SchemaMigration[] = [
  ...
  },
  {
    ...
    up(db) {
      db.run("CREATE INDEX events_type_created ON events(type, created_at DESC)");
    },
  },
  {
    id: "0010",
  ...
  },
  {
    ...
    up(db) {
      if (!hasColumn(db, "account_activities", "corporate_action")) {
        db.run("ALTER TABLE account_activities ADD COLUMN corporate_action TEXT");
      }
...
export function migrateDatabase(db: Database, migrations: readonly SchemaMigration[] = SCHEMA_MIGRATIONS) {
  for (let index = 1; index < migrations.length; index += 1) {
    if (migrations[index - 1]!.id >= migrations[index]!.id) throw new Error("Database migrations must be strictly ordered");
  }
  ...
  const applied = db.query("SELECT id, name, checksum FROM schema_migrations ORDER BY id").all() as Pick<SchemaMigration, "id" | "name" | "checksum">[];
  for (const row of applied) {
    const expected = known.get(row.id);
    if (!expected) throw new Error(`Unknown database migration ${row.id}`);
    if (row.name !== expected.name || row.checksum !== expected.checksum) throw new Error(`Database migration ${row.id} does not match the application`);
  }
  if (applied.some((row, index) => row.id !== migrations[index]?.id)) throw new Error("Database migration history is not contiguous");

  const appliedIds = new Set(applied.map(row => row.id));
  const insert = db.query("INSERT INTO schema_migrations (id, name, checksum) VALUES (?, ?, ?)");
