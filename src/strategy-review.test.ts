...
import { parseStrategyReview, strategyReviewStatus, withStrategyReviewConfig } from "./strategy-review";
...
test("parses a strategy review with actor, timestamp and revision evidence", () => {
  const parsed = parseStrategyReview(
    { action: "revise", note: "Widen the spread gate after paper slippage review", revision: { maxSpreadBps: 75 } },
    "tester",
    "paper",
...
test("appends review history without mutating unrelated config", () => {
  const next = withStrategyReviewConfig(
    { strategyId: "moving-average-trend", reviewHistory: [{ action: "continue", note: "old" }] },
    { action: "promote", actor: "tester", reviewedAt: "2026-06-25T11:00:00.000Z", note: "Promote after beating cash and buy-and-hold" },
  );
