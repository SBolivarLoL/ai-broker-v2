...
import { benchmarkAttribution, diversificationScore, moneyWeightedReturn, performancePoints, performanceSummary, stressTests, timeWeightedReturn, valueAtRisk95 } from "./analytics";
...
describe("portfolio analytics", () => {
  ...
  test("normalizes broker history and summarizes performance", () => {
    expect(points.at(-1)).toMatchObject({ timestamp: 3000, equity: 99, profitLossPercent: -1, externalCashFlow: 0 });
  ...
  test("cashflow-adjusts TWR and calculates annualized money-weighted return", () => {
    ...
    const funded = performancePoints({ timestamp: [1, halfYear, halfYear * 2], equity: [100, 210, 231], profitLoss: [0, 10, 31], profitLossPct: [0, .1, .21], cashflow: { JNLC: [0, 100, 0] } });
    expect(timeWeightedReturn(funded)).toBeCloseTo(.21);
    expect(performanceSummary(funded).timeWeightedReturnPercent).toBeCloseTo(21);
    expect(moneyWeightedReturn(funded)).not.toBeNull();
  });

  test("attributes cashflow-adjusted portfolio return against a benchmark", () => {
    const day = 86_400;
    const funded = performancePoints({ timestamp: [day, day * 2, day * 3], equity: [100, 210, 231], profitLoss: [0, 10, 31], profitLossPct: [0, .1, .21], cashflow: { JNLC: [0, 100, 0] } });
    const benchmark = benchmarkAttribution(funded, [{ timestamp: day * 1_000, close: 100 }, { timestamp: day * 3_000, close: 110 }], "SPY");
    ...
    expect(benchmarkAttribution(funded, [], "SPY").quality).toBe("insufficient");
  });

  test("reconciles delayed opening funding to the first funded equity point", () => {
    const halfYear = 182 * 86_400;
    const delayed = performancePoints({ timestamp: [1, 86_400, 86_400 + halfYear, 86_400 + halfYear * 2], equity: [0, 100, 101, 102], profitLoss: [0, 0, 1, 2], profitLossPct: [0, 0, .01, .02], cashflow: { JNLC: [0, 0, 0, 100] } });
    expect(delayed.map(point => point.externalCashFlow)).toEqual([100, 0, 0]);
    expect(timeWeightedReturn(delayed)).toBeCloseTo(.02);
