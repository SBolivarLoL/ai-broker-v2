...
test("attributes filled strategy paper orders across post-fill windows", () => {
  ...
  expect(order).toMatchObject({ paperOrderId: "paper-1", fillQuality: { status: "available", slippageBps: 20 } });
...
test("keeps unfilled and missing market data explicit", () => {
  const attribution = buildStrategyOrderAttribution({
    ...
    orders: [{
      ...
      payload: { symbol: "ETH/USD", side: "buy", notional: 50, referencePrice: 2_500, broker: { status: "accepted" } },
    }],
    barsBySymbol: {},
  });

  expect(attribution.summary).toMatchObject({ filledOrders: 0, ordersWithFillQuality: 0, attributedWindows: 0, averageSlippageBps: null });
  expect(attribution.orders[0]!.windows[0]).toMatchObject({ window: "1h", status: "not_filled" });
  expect(attribution.warnings).toContain("No filled strategy paper orders are available for attribution yet.");
});
