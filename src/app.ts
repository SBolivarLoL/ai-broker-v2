...
function secIdentityConfigured() {
  try { secUserAgentFromEnv(env); return true; }
  catch { return false; }
}
...
async function pollStrategyScheduler() {
  ...
  async function fetch(request: Request) {
    ...
    if (!["GET", "HEAD", "OPTIONS"].includes(request.method) && !validMutationOrigin(request, env)) return json({ error: "Invalid request origin" }, 403);
    ...
    try {
      if (url.pathname === "/") return new Response(Bun.file(indexPath), { headers: { ...securityHeaders, "cache-control": "no-store" } });
      ...
      if (url.pathname === "/ready") {
        if (previewSecret.length < 32 || !securityReady(env) || !secIdentityConfigured()) return json({ status: "not_ready", error: "Security or external-data identity configuration is incomplete" }, 503);
      ...
      if (url.pathname === "/api/portfolio/performance") {
        ...
        const benchmarkSymbol = (env.PORTFOLIO_BENCHMARK ?? "SPY").trim().toUpperCase();
      ...
      if (url.pathname === "/api/agent/plans" && request.method === "POST") {
        ...
        if (!env.OPENAI_API_KEY) return json({ error: "Add OPENAI_API_KEY to .env to enable the agent" }, 503);
      ...
      if (url.pathname === "/api/agent/questions" && request.method === "POST") {
        ...
        if (!env.OPENAI_API_KEY) return json({ error: "Add OPENAI_API_KEY to .env to enable portfolio Q&A" }, 503);
      ...
      if (url.pathname === "/api/research/runs" && request.method === "POST") {
        ...
        if (!env.OPENAI_API_KEY) return json({ error: "Add OPENAI_API_KEY to .env to enable company research" }, 503);
        const symbol = String((await requestJson(request)).symbol ?? "").trim().toUpperCase();
        if (!/^[A-Z.]{1,10}$/.test(symbol)) return json({ error: "A valid stock symbol is required" }, 400);
        const runId = crypto.randomUUID(); const model = env.OPENAI_MODEL ?? "gpt-5.5";
