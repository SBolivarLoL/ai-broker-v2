...
export function companyMarketSnapshot(asset: any, snapshot: any, bars: any[], news: any[], clock: any, period: string) {
  ...
  return {
    ...
    stats: { dayChangePercent, periodReturnPercent: normalizedBars.length > 1 ? (normalizedBars.at(-1)!.close / normalizedBars[0].close - 1) * 100 : null, periodHigh: normalizedBars.length ? Math.max(...normalizedBars.map(bar => bar.high)) : null, periodLow: normalizedBars.length ? Math.min(...normalizedBars.map(bar => bar.low)) : null, currentVolume, averageVolume20d, relativeVolume: currentVolume !== null && averageVolume20d ? currentVolume / averageVolume20d : null },
    bars: normalizedBars,
