...
test("normalizes company price, spread, volume and news", () => {
  ...
  expect(result.benchmark).toMatchObject({ symbol: "SPY", observations: 2, quality: "complete" });
...
test("reports insufficient benchmark coverage without inventing relative strength", () => {
  const result = companyMarketSnapshot(asset, {}, bars, [], {}, "1M", "QQQ", benchmarkBars.slice(0, 1));
  expect(result.benchmark).toMatchObject({ symbol: "QQQ", returnPercent: null, relativeStrengthPercent: null, quality: "insufficient" });
});
