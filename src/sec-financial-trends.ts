...
function cadenceEntry(entry: FactEntry, kind: MetricDefinition["kind"], cadence: SecFinancialCadence) {
  if (!Number.isFinite(entry.val) || !/^\d{4}-\d{2}-\d{2}$/.test(entry.end) || !entry.accn) return false;
  ...
  return cadence === "annual" ? days >= 300 && days <= 400 : days >= 70 && days <= 120;
}
...
function observation(company: SecCompany, concept: string, unit: string, cadence: SecFinancialCadence, entry: FactEntry): SecFinancialObservation {
  return {
    value: entry.val,
    unit,
    cadence,
...
function observations(company: SecCompany, concept: string, unit: string, kind: MetricDefinition["kind"], entries: FactEntry[], cadence: SecFinancialCadence, limit: number) {
  const byPeriod = new Map<string, FactEntry[]>();
  for (const entry of entries.filter(item => cadenceEntry(item, kind, cadence))) {
    const records = byPeriod.get(entry.end) ?? [];
    records.push(entry);
    byPeriod.set(entry.end, records);
  }
  return [...byPeriod.values()]
    .map(records => records.toSorted((a, b) => b.filed.localeCompare(a.filed) || b.form.localeCompare(a.form))[0]!)
...
export function buildSecFinancialTrends(company: SecCompany, facts: SecFacts, annualLimit = 4, quarterlyLimit = 8): SecFinancialTrends {
  if (!Number.isInteger(annualLimit) || annualLimit < 1 || annualLimit > 10) throw new Error("SEC annual trend limit must be between 1 and 10");
  if (!Number.isInteger(quarterlyLimit) || quarterlyLimit < 1 || quarterlyLimit > 20) throw new Error("SEC quarterly trend limit must be between 1 and 20");
  const metrics = METRICS.flatMap(definition => {
    const source = metricFacts(facts, definition);
    if (!source) return [];
    const annual = observations(company, source.concept, definition.unit, definition.kind, source.entries, "annual", annualLimit);
    const quarterly = observations(company, source.concept, definition.unit, definition.kind, source.entries, "quarterly", quarterlyLimit);
    if (!annual.length && !quarterly.length) return [];
    return [{ id: definition.id, label: definition.label, concept: source.concept, unit: definition.unit, kind: definition.kind, annual, quarterly }];
  });
  const all = metrics.flatMap(metric => [...metric.annual, ...metric.quarterly]);
  return {
    ...
    coverage: {
      ...
      annualObservations: metrics.reduce((sum, metric) => sum + metric.annual.length, 0),
      quarterlyObservations: metrics.reduce((sum, metric) => sum + metric.quarterly.length, 0),
      latestPeriodEnd: all.toSorted((a, b) => b.periodEnd.localeCompare(a.periodEnd))[0]?.periodEnd ?? null,
    },
    limitations: [
