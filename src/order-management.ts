...
const working = new Set(["new", "accepted", "pending_new", "pending_cancel", "pending_replace", "accepted_for_bidding", "partially_filled", "held", "calculated", "stopped", "suspended"]);

export function canCancelOrder(status: unknown) {
  return typeof status === "string" && cancellable.has(status);
}

export function canReplaceOrder(order: Pick<trading.Order, "status" | "type" | "notional" | "qty">) {
  return typeof order.status === "string" && replaceable.has(order.status) && order.notional === null && Number.isInteger(Number(order.qty)) && ["limit", "stop", "stop_limit", "trailing_stop"].includes(order.type);
}
...
export const ReplacementPreview = z.object({
  orderId: z.string().uuid(), symbol: z.string().min(1), side: z.enum(["buy", "sell"]), expectedUpdatedAt: z.string().nullable(),
  original: ReplacementInput, replacement: ReplacementInput, expiresAt: z.number().int(),
});
export type ReplacementPreview = z.infer<typeof ReplacementPreview>;

const numeric = (value: string | null | undefined) => value === null || value === undefined ? null : Number(value);

export function buildReplacementPreview(order: trading.Order, raw: unknown, expiresAt: number): ReplacementPreview {
  if (!order.id || !order.symbol || !order.side || !canReplaceOrder(order)) throw new Error("This order is not currently replaceable");
  const replacement = ReplacementInput.parse(raw);
  const original = ReplacementInput.parse({ qty: Number(order.qty), limitPrice: numeric(order.limitPrice), stopPrice: numeric(order.stopPrice) });
  const filled = Number(order.filledQty ?? 0);
  if (replacement.qty > original.qty || replacement.qty <= filled) throw new Error("Replacement quantity must reduce or preserve the order and remain above the filled quantity");
  if (["limit", "stop_limit"].includes(order.type) && replacement.limitPrice === null) throw new Error("A limit price is required");
  if (["stop", "stop_limit"].includes(order.type) && replacement.stopPrice === null) throw new Error("A stop price is required");
  const improves = (next: number | null, prior: number | null) => next === null || prior === null || (order.side === "buy" ? next <= prior : next >= prior);
  if (!improves(replacement.limitPrice, original.limitPrice) || !improves(replacement.stopPrice, original.stopPrice)) throw new Error("Replacement prices may not worsen the original order's exposure");
  if (replacement.qty === original.qty && replacement.limitPrice === original.limitPrice && replacement.stopPrice === original.stopPrice) throw new Error("At least one order field must change");
...
export class OrderTracker {
  ...
  recover(orders: trading.Order[], now = new Date()) {
    for (const order of orders) {
      ...
      const streamedAt = Number(streamed?.updatedAt ?? streamed?.submittedAt ?? 0), recoveredAt = Number(order.updatedAt ?? order.submittedAt ?? 0);
      if (!streamed || recoveredAt >= streamedAt) this.orders.set(order.id, order);
    }
    this.lastRecoveryAt = now.toISOString();
  }
  update(order: trading.Order, now = new Date()) { if (order.id) this.orders.set(order.id, order); this.lastEventAt = now.toISOString(); }
  setStreamState(state: string, error: string | null = null) { this.streamState = state; this.lastError = error; }
  list(status: "open" | "closed" | "all", limit: number) {
    return [...this.orders.values()].filter(order => status === "all" || (status === "open") === working.has(String(order.status)))
      .sort((a, b) => Number(b.submittedAt ?? b.createdAt ?? 0) - Number(a.submittedAt ?? a.createdAt ?? 0)).slice(0, limit);
  }
  metadata(now = Date.now()) {
    const recoveryAgeMs = this.lastRecoveryAt ? now - Date.parse(this.lastRecoveryAt) : null;
    return { streamState: this.streamState, lastEventAt: this.lastEventAt, lastRecoveryAt: this.lastRecoveryAt, recoveryAgeMs, stale: recoveryAgeMs === null || (this.streamState !== "authenticated" && recoveryAgeMs > 90_000), lastError: this.lastError };
  }
