...
test("exposes upcoming sessions, early closes and order timing guidance", () => {
  const clock = { clocks: [{ market: { acronym: "NASDAQ" }, phase: "post", nextMarketOpen: new Date("2026-06-23T13:30:00Z"), nextMarketClose: new Date("2026-06-23T20:00:00Z") }] };
  expect(orderSessionGuidance(clock)).toMatchObject({ phase: "post", coreSession: false, nextOpen: "2026-06-23T13:30:00.000Z" });
  const result = calendarDto({ market: { name: "Nasdaq", acronym: "NASDAQ", timezone: "America/New_York" }, calendar: [{ date: new Date("2026-07-03"), coreStart: new Date("2026-07-03T13:30:00Z"), coreEnd: new Date("2026-07-03T17:00:00Z"), settlementDate: new Date("2026-07-07") }] }, clock);
  expect(result.sessions[0]).toMatchObject({ date: "2026-07-03", durationMinutes: 210, earlyClose: true, settlementDate: "2026-07-07" });
});
